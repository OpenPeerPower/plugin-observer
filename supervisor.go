...
func supervisorLogs(w io.Writer) error {
	...
	reader, err := cli.ContainerLogs(context.Background(), "oppio_supervisor", types.ContainerLogsOptions{
		ShowStdout: true,
		ShowStderr: true,
		Follow:     false,
		Timestamps: false,
...
func supervisorRestart() error {
	// Read logs from container
	err := cli.ContainerStop(context.Background(), "oppio_supervisor", nil)
	if err != nil {
